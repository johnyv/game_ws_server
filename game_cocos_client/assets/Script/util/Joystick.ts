...
export default class Joystick extends cc.Component {
  ...
  _touchMoveEvent(event: cc.Event.EventTouch) {
    ...
    let speedType;
    let offset:number = this.dot.width/2;
    if (this._radius > (distance + offset)) {
      this.dot.setPosition(cc.v2(posX, posY));

      speedType = SpeedType.NORMAL;
    } else {
      // 控杆永远保持在圈内，并在圈内跟随触摸更新角度
      const x = this._stickPos.x + p.x * (this._radius - offset);
      const y = this._stickPos.y + p.y * (this._radius - offset);
